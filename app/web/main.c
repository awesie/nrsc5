...
static void *server_scan_worker(void *arg)
{
    ...
    while (nrsc5_scan(server->radio, server->frequency, NRSC5_SCAN_END, NRSC5_SCAN_SKIP, &server->frequency) == 0)
    {
...
static void log_to_lwsl(int level, const char *file, int line, const char *fmt, ...)
{
    int filter;
    va_list ap;
    ...
    {
    default:
    case NRSC5_LOG_TRACE:
    case NRSC5_LOG_DEBUG:
        filter = LLL_DEBUG;
        break;
    case NRSC5_LOG_INFO:
        filter = LLL_INFO;
    ...
    case NRSC5_LOG_WARN:
        filter = LLL_WARN;
        break;
    case NRSC5_LOG_ERROR:
    case NRSC5_LOG_FATAL:
        filter = LLL_ERR;
        break;
    }

    va_start(ap, fmt);
    _lws_logv(filter, fmt, ap);
    va_end(ap);
}

int main(int argc, char *argv[])
{
    int opt, n = 0, device_index = 0, ppm_error = 0;
    char *webroot;
    ...
    while ((opt = getopt(argc, argv, "d:p:v")) != -1)
    {
        ...
        {
        case 'd':
            device_index = strtol(optarg, NULL, 10);
            break;
        case 'p':
            ppm_error = strtol(optarg, NULL, 10);
            break;
        case 'v':
            printf("nrsc5-web revision %s\n", GIT_COMMIT_HASH);
            return 0;
        default:
            goto usage;
        }
    ...
    {
usage:
        fprintf(stderr, "Usage: %s [-d device] [-p ppm-correction] [-v] webroot\n", argv[0]);
        return 1;
    }

    webroot = argv[optind++];
    asprintf(&server->static_path, "%s/static", webroot);
    asprintf(&server->cache_path, "%s/cache", webroot);
    mkdir(server->cache_path, 0700);

    pthread_mutex_init(&server->mutex, NULL);
    pthread_mutex_init(&server->lws_mutex, NULL);

    nrsc5_set_logger(log_to_lwsl);
    if (nrsc5_open(&server->radio, device_index, ppm_error) != 0)
    {
